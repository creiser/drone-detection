...
from cntk.io import MinibatchSource, ImageDeserializer, CTFDeserializer, transforms, StreamDefs, StreamDef
...
def create_mb_source(img_height, img_width, img_channels, n_classes, n_rois, data_path, data_set):
    ...
    image_source = ImageDeserializer(map_file, StreamDefs(features = StreamDef(field='image', transforms=image_transforms)))

    # read rois and labels
    roi_source = CTFDeserializer(roi_file, StreamDefs(rois = StreamDef(field='rois', shape=rois_dim, is_sparse=False)))
    label_source = CTFDeserializer(label_file, StreamDefs(roiLabels = StreamDef(field='roiLabels', shape=label_dim, is_sparse=False)))
...
if __name__ == '__main__':
    os.chdir(base_path)
    model_path = os.path.join(cntkFilesDir, "Output", "frcn_py.model")
    ...
    #eval_model = combine([trained_model.outputs[2].owner])

    # Evaluate the test set
    #test_fast_rcnn(eval_model)
